...
| Prop | Description | Default |
|---|---|---|
|**`color`**|Text and icon color, use `iconStyle` or `textStyle` if you need different colors.|`white`|
|**`size`**|Icon size.|`20`|
|**`iconStyle`**|Styles applied to the icon only, good for setting margins or a different color.|`{marginRight: 10}`|
|**`textStyle`**|Styles applied to the text only, good for setting `fontSize` or `fontFamily`.|`{fontWeight: '600'}`|
